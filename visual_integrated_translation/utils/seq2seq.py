...
def rnn_decoder(decoder_inputs, initial_state, cell, loop_function=None,
  ...
  with vs.variable_scope(scope or "rnn_decoder"):
    ...
    for i in xrange(len(decoder_inputs)):
      ...
      output, new_state = cell(inp, states[-1])  #### if cell=GRUCell(input, state_before) -> return new_hidden, new_hidden
  ...
  with vs.variable_scope(scope or "embedding_rnn_seq2seq"):
    ...
    if isinstance(feed_previous, bool):
      ...
      outputs = control_flow_ops.cond(feed_previous,
                                      lambda: outputs1, lambda: outputs2) #Return either 'outputs1' or 'outputs2' based on the boolean predicate 'feed_previous'
...
def embedding_attention_seq2seq(encoder_inputs, decoder_inputs, cell,
  ...
  with vs.variable_scope(scope or "embedding_attention_seq2seq"):
    ...
    if isinstance(feed_previous, bool): ##### this in seq2seq_model (attention model)
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
  ...
  Args:
    ...
    num_decoder_symbols: integer, number of decoder symbols (output classes). = target_vocab_size
    ...
    for j in xrange(len(buckets)):
      with vs.variable_scope(vs.get_variable_scope(),
        ...
        bucket_outputs, _ = seq2seq(bucket_encoder_inputs,
                                    bucket_decoder_inputs) ### "Hello!" bucket_encoder_inputs=[[0][0][0][21][322]], bucket_decoder_inputs=[[1][0][0]...]
                                                           ### in seq2seq_model.py  seq2seq() = embedding_attention_seq2seq()
